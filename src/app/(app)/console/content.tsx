...
import { useStreamingQuery } from '@/lib/core/hooks/use-streaming-query'
...
export default function ConsolePageContent() {
  ...
  useEffect(() => {
    ...
    if (previousGraphId.current && previousGraphId.current !== graphId) {
      ...
      addSystemMessage(
        `═══════════════════════════════════════════════════════════════\n` +
          `Portfolio context changed: ${previousGraphId.current} → ${graphId}\n` +
          `═══════════════════════════════════════════════════════════════\n\n` +
          `Console has been reset for the new portfolio context.\n` +
  ...
  const getWelcomeMessage = (): string => {
    return (
      `RoboInvestor Console v${apiVersion} - Portfolio: ${graphId || 'Not selected'}\n` +
      `═══════════════════════════════════════════════════════════════\n\n` +
      `Claude-powered interactive investment analysis console\n\n` +
      `USAGE:\n` +
      ...
      `    /query MATCH (h:Holding) RETURN h.symbol, h.shares LIMIT 10\n` +
      `    /query MATCH (t:Transaction) RETURN t.date, t.type ORDER BY t.date DESC\n\n` +
      `COMMANDS:\n` +
      ...
      `How can I help you analyze your investments today?`
    )
  }
  ...
  const executeCypherQuery = async (cypherQuery: string) => {
    if (!graphId) {
      addErrorMessage('No portfolio selected. Please select a portfolio first.')
      return
    }
    ...
    try {
      ...
      addSystemMessage(
        `MCP Setup Instructions:\n` +
          ...
          `       "roboinvestor": {\n` +
          `         "command": "npx",\n` +
          `         "args": ["-y", "@roboinvestor/mcp"],\n` +
          `         "env": {\n` +
          `           "ROBOSYSTEMS_API_URL": "${apiUrl}",\n` +
          `           "ROBOSYSTEMS_API_KEY": "${apiKey}",\n` +
          `           "ROBOSYSTEMS_GRAPH_ID": "${graphId || 'your_portfolio_id'}"\n` +
          `         }\n` +
          `       }\n` +
          `     }\n` +
          `   }\n\n` +
