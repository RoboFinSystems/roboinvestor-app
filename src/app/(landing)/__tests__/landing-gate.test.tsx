...
vi.mock('../content', () => ({
  default: () => <h1>Research every holding.</h1>,
}))
vi.mock('../maintenance', () => ({
...
describe('LandingGate', () => {
  ...
  it('sends crawlers the content in the server HTML, with nothing marking it hidden', () => {
    ...
    expect(html).toContain('<h1>Research every holding.</h1>')
  ...
  it('uncovers the content for a signed-out visitor', () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Research every holding.' })
    ).toBeInTheDocument()
