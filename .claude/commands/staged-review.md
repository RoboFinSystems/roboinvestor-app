...
Shared UI comes from the `@robosystems/core` and `@robosystems/report-components` npm packages. A staged edit that patches around one of those components' behavior — rather than composing it — will be overwritten by the next version bump. Flag it and point the fix upstream. The same goes for portfolio or SEC data logic that belongs in the RoboSystems API rather than the presentation layer — `src/lib/sec/` is a viewer over the shared SEC repository, not its source of truth.

`src/lib/` and `src/app/api/` are **per-app copies** shared by name with robosystems-app and roboledger-app, not by package. A fix to `rate-limiter.ts`, `client-ip.ts`, `turnstile-server.ts`, or `sns.ts` here probably needs hand-porting to the sibling apps — note it in the review.

## Application context (`src/app/`, `src/components/`, `src/lib/`)

**Portfolio correctness (read this before the style points):**

- Does anything change a displayed figure — positions, holdings, valuations, returns, as-of dates, currency or precision handling? A presentation change that silently alters a number is this app's worst failure mode, and it needs a test, not just a reviewer's eye.
- Is money handled without float arithmetic, and are as-of dates derived rather than assumed?
...
- Is coverage maintained on critical paths (auth, portfolio rendering, SEC report viewing)?
...
- Do CloudFormation edits require a stack update, and which stack (`RoboInvestorApp{Staging,Prod}` vs `RoboInvestorAppS3{Staging,Prod}`)?
- Are secrets referenced rather than inlined? Nothing resembling a credential should appear in the diff at all.

## Public-repo hygiene

- No customer names, graph IDs, real holdings or position sizes, or internal cost/pricing detail in code comments or fixtures. Portfolio data is personal financial data; test fixtures built from a real account are the recurring offender — figures should be invented, not borrowed.
