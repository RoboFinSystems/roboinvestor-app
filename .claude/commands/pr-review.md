...
- **URL provided** (e.g., `https://github.com/RoboFinSystems/roboinvestor-app/pull/143`): Extract the repo and PR number
- **Number provided** (e.g., `143`): Use the current repository
...
- **Right repo**: Is this app-local code, or a patch over something that belongs in `@robosystems/core` or `@robosystems/report-components`? A fix applied here to a shared component gets overwritten by the next version bump — that's a blocking issue, not a nit. Same for portfolio or SEC data behavior that belongs in the RoboSystems API rather than the presentation layer.
- **Portfolio correctness**: does anything change a displayed figure — positions, holdings, valuations, returns, as-of dates, currency or precision handling? Those need explicit scrutiny and a test; a rendering change that silently alters a number is the worst failure mode this app has.
