...
- **Both environments share one concurrency group** (`roboinvestor-app-deploy`, `cancel-in-progress: false`). A staging deploy queues behind an in-flight prod deploy and vice versa. Don't fire a second deploy expecting it to preempt the first, and check for a queued run before concluding a deploy "isn't starting."
...
- **`deploy-s3` (CloudFormation)** — the static-assets stack (`RoboInvestorAppS3{Staging,Prod}`). The dangerous class: a stack left in a rollback state usually **can't be updated again** until it's resolved, so a naive re-run fails identically. Read the stack events before re-triggering:
  ```bash
  aws cloudformation describe-stack-events --stack-name RoboInvestorAppS3Prod --max-items 20
  ```
...
- **`deploy-app`** — the main stack (`RoboInvestorApp{Staging,Prod}`) plus the App Runner rollout. Failures split three ways: a CloudFormation update failure, App Runner never reaching `RUNNING` (bad image, failed container start), or the final health check never returning 200. The step names in the log tell you which.
...
npm run deploy:prod -- v0.3.11      # bin/deploy.sh prod <tag> — always name the tag
# equivalently: gh workflow run prod.yml --ref v0.3.11
```
...
```bash
curl -sf https://roboinvestor.ai/api/utilities/health && echo OK   # prod, via CloudFront
curl -sf https://staging.roboinvestor.ai/api/utilities/health && echo OK
```
