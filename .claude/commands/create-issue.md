Create a GitHub issue for the roboinvestor-app repository based on the user's input.
...
6. **Set Issue Type** - After creation, set the issue type via GraphQL:
   ```bash
   # Get issue ID
   gh api graphql -f query='{ repository(owner: "RoboFinSystems", name: "roboinvestor-app") { issue(number: NUMBER) { id } } }'
