...
1. **Check you're in the right repo first** - This app consumes shared UI from the [`@robosystems/core`](https://www.npmjs.com/package/@robosystems/core) and `@robosystems/report-components` npm packages, and talks to the RoboSystems API. Before filing here, decide where the work actually lands:
   ...
   - A bug in API behavior, portfolio or securities data, the extensions GraphQL surface, or an endpoint contract belongs in `RoboFinSystems/robosystems`. That includes wrong or missing SEC filing data — `src/lib/sec/` is a viewer over the shared SEC repository, not its source of truth.
...
4. **Draft the Issue** - Read the matching YAML template in `.github/ISSUE_TEMPLATE/` and mirror its structure. Each template declares its own `type:` in frontmatter and marks which fields are required — read the file rather than guessing the sections. Fill the optional fields too where you have the information; they're the ones that make an issue actionable later.
   ...
   For a UI bug, the reproduction should name the route (e.g. `/portfolio`, `/research`, `/entity`), the viewport if it's responsive-specific, and the color scheme if it's dark-mode-specific. Those three are the details most often missing and most often the whole bug.

   For a **numbers** bug — a position that doesn't reconcile, a valuation that disagrees with the source — say which holding, which as-of date, and what the expected value was. Without those, "the portfolio is wrong" isn't reproducible.
...
5. **Sanitize for Public Visibility** - This repo is public and the issue is world-readable immediately. Before creating:
   ...
   - Remove specific customer names, graph IDs, or account data. This app displays real portfolios, so screenshots are the usual leak — holdings, position sizes, and account values are personal financial data and must never appear in a public issue. Check pasted console output for session tokens, API keys, and email addresses too. Redact or reconstruct with dummy figures rather than blurring.
...
User: "The holdings table header overlaps the totals on mobile"
